...
def copy_dir():
    if _platform == "win32":
        subprocess.call(['robocopy',sys.argv[1], destination_final_path, '/E'])
    elif _platform == "darwin":
        # https://github.com/amiaopensource/ltopers/blob/master/writelto#L51
        cmd = ['gcp','--preserve=mode,timestamps', '-nRv',sys.argv[1], destination_final_path]
        subprocess.call(cmd)
    elif _platform == "linux2":
        # https://github.com/amiaopensource/ltopers/blob/master/writelto#L51
        cmd = [ 'cp','--preserve=mode,timestamps', '-nRv',sys.argv[1], destination_final_path]
        print cmd
        subprocess.call(cmd)
